...
if (!class_exists(Kernel::class) || (Kernel::MAJOR_VERSION >= 7 || Kernel::MAJOR_VERSION === 6 && Kernel::MINOR_VERSION === 4)) {
    class MangaReviewReactionsNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
    {
        ...
        {
            return is_object($data) && $data instanceof MangaReviewReactions;
        }
        ...
        public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
        {
            ...
            $mangaReviewReactions = new MangaReviewReactions();
            if (null === $data || !\is_array($data)) {
                return $mangaReviewReactions;
            }

            if (\array_key_exists('overall', $data)) {
                $mangaReviewReactions->setOverall($data['overall']);
                unset($data['overall']);
            }

            if (\array_key_exists('nice', $data)) {
                $mangaReviewReactions->setNice($data['nice']);
                unset($data['nice']);
            }

            if (\array_key_exists('love_it', $data)) {
                $mangaReviewReactions->setLoveIt($data['love_it']);
                unset($data['love_it']);
            }

            if (\array_key_exists('funny', $data)) {
                $mangaReviewReactions->setFunny($data['funny']);
                unset($data['funny']);
            }

            if (\array_key_exists('confusing', $data)) {
                $mangaReviewReactions->setConfusing($data['confusing']);
                unset($data['confusing']);
            }

            if (\array_key_exists('informative', $data)) {
                $mangaReviewReactions->setInformative($data['informative']);
                unset($data['informative']);
            }

            if (\array_key_exists('well_written', $data)) {
                $mangaReviewReactions->setWellWritten($data['well_written']);
                unset($data['well_written']);
            }

            if (\array_key_exists('creative', $data)) {
                $mangaReviewReactions->setCreative($data['creative']);
                unset($data['creative']);
            }

            foreach ($data as $key => $value) {
                if (preg_match('#.*#', (string) $key)) {
                    $mangaReviewReactions[$key] = $value;
                }
            }
    ...
    class MangaReviewReactionsNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
    {
         ...
         */
        public function denormalize($data, $type, $format = null, array $context = []): Reference|MangaReviewReactions
        {
            if (isset($data['$ref'])) {
                return new Reference($data['$ref'], $context['document-origin']);
            }
